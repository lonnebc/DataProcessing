...
def scrape_movie_page(dom):
    ...
    actors = ""
    actors_empty = True
    # scrape  title  and duration
    title = dom.by_class("header")[0].by_class("itemprop")[0].content
    duration = dom.by_attr(itemprop="duration")[0].content[:-5].strip()
    # scrape genres, could be more than 1
    genres = str("")
    gnre = dom.by_class("infobar")[0]
    for genre in gnre.by_attr(itemprop="genre"):
        genres += str(genre[0])
        genres += "; "
    genres = genres[:-2]
    # get director
    ...
    # get actors
    for actrs in dom.by_attr(itemprop="actors")[:3]:
        for j in actrs.by_attr(itemprop="name"):
